	...
	switch(geneticSettings.getParentsSelectionType())
	{
        case PANMIXIA:
        {
            ...
            for(int i = 0; i < individualsNum / 2; ++i)
            {
                ...
                geneticSettings.getRecombination() -> process(individuals[firstParent]->getDna(), individuals[secondParent]->getDna(),
                                                              nextIndividuals[i * 2]->getDna(), nextIndividuals[i * 2 + 1]->getDna());
            }
            break;
        }
